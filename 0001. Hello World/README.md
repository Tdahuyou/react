...
- [📒 API 描述](#-api-描述)
- [💻 demo - 在页面上渲染出 Hello World](#-demo---在页面上渲染出-hello-world)
- [💻 demo - 熟悉 React.createElement 的参数](#-demo---熟悉-reactcreateelement-的参数)
- [💻 demo - 使用 JSX 来简写](#-demo---使用-jsx-来简写)
...
- [🤔 思考：为什么一旦使用了 `JSX` 语法，就必须要引入 `react` 核心库？](#🤔-思考为什么一旦使用了-`jsx`-语法就必须要引入-`react`-核心库)
- [🤔 思考：通过脚手架（比如 vite、umi、create-react-app）来搭建工程 vs. 通过（.html）页面的方式来直接引入 react 相关的库](#🤔-思考通过脚手架比如-vite、umi、create-react-app来搭建工程-vs-通过html页面的方式来直接引入-react-相关的库)
